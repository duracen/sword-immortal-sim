...
const MASTERS = [
  ...
  { key: '분혼', name: '분혼마주', trigger: '신통/법보 첫 cast 시 발동, CD 160초', cd: 160 },
  { key: '식혼', name: '식혼마주', trigger: '공격 후 사망 시 발동, CD 160초', cd: 160 },
  { key: '악신', name: '악신마주', trigger: '방어법보 파괴 시 (또는 4회 공격), CD 170초', cd: 170 },
  { key: '혼원', name: '혼원마주', trigger: '방어법보 파괴 시, CD 160초 (너프 적용 2026-05-09)', cd: 160 },
...
const MASTER_BRANCH_DESC = {
  ...
  혼원: {
    무: '※ 너프 적용 (2026-05-09)\n\n방어법보가 파괴될 때 인과 역전이 발동되어 해당 법보를 복구하고, 보호막을 획득하며, 그 수치는 모든 방어법보 보호막 평균값의 90.00%이며, 발동 후 재사용 시간에 진입한다.\n\n(이전 사양: 평균값 + 10% 추가 = 110%)',
    허: '방어법보가 파괴되면 인과 역행이 발동되어, 해당 법보를 복구하고 모든 방어법보의 호신강기 평균값의 60.00%에 해당하는 호신강기를 획득하며, 12초간 신통 및 치명타 차단을 48.00% 추가 획득한다.\n\n(변경 없음)',
    진: '※ 너프 적용 (2026-05-09)\n\n방어법보가 파괴될 때 인과 역전이 발동하여 해당 법보를 수리하고 보호막을 획득합니다. 보호막 수치는 모든 방어법보 보호막 평균값의 76.00%입니다. 자신의 모든 방어법보의 미파괴 효과는 모든 방어법보가 파괴될 때까지 유지됩니다.\n\n(이전 사양: 평균값의 93%)',
  },
  업화: {
...
export default function BisulPicker({ value, onChange }) {
  ...
  // 단일 선택 — 자기와 더미(상대) 동일 적용 (사용자가 끼는 비술 = 더미도 동일하게 끼고 있는 것으로 sim)
  function handleChange(list) {
    onChange({ self: list, enemy: list });
  }
  return (
