...
export default function WinnerPodium({ results, sortBy }) {
  const key = `s${sortBy}`;
  const top3 = useMemo(() => [...results].sort((a, b) => { if (a.killTime != null || b.killTime != null) { const _x = a.killTime != null ? a.killTime : Infinity, _y = b.killTime != null ? b.killTime : Infinity; if (_x !== _y) return _x - _y; } return (b[key] ?? 0) - (a[key] ?? 0); }).slice(0, 3), [results, key]);
    ...
    <div className="space-y-4">
      ...
      >
        ...
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 sm:gap-3 mb-3">
          {[
            { label: '41초 (1사이클)', v: winner.s41, hl: sortBy === '41' },
            { label: '52초', v: winner.s52, hl: sortBy === '52' },
