...
function newState() {
  return {
    ...
    // 빙의 시작/종료 시각 + CD 추적
    법상_빙의시작T: -Infinity,
    법상_빙의종료T: -Infinity,  // 빙의 active 종료 시각 (이 시각 이후 cleanup 발동)
    법상_lastCdEnd: -Infinity,  // 다음 빙의 가능 시각
    법상_빙의시작처리: false,    // 빙의 시작 시 1회 cleanup 트리거 처리
    법상_빙의종료처리: false,    // 빙의 종료 시 1회 cleanup 트리거 처리
    ...
    법상_적혼: 0,     // 주작 (자기 입히는 +5, 받는 -5, max 5)
    법상_진룡각인: 0, // 진룡 (자기 입히는 +5, 받는 -5, max 5)
    법상_봉황각인: 0, // 봉황 (적 받는 +5, 입히는 -5, max 5)
    // 법상 timing/카운터
    법상_금오피해카운터: 0,
    법상_금오마지막발사T: -Infinity,
    법상_주작마지막발사T: -Infinity,
    법상_봉황마지막발사T: -Infinity,
    법상_진룡공격카운터: 0,  // 의념: 2회마다 청령
    법상_진룡각인_누적: 0,  // 실체: 누적 3회 획득마다 700%
    법상_적난새의념피해횟수: 0,  // 진령: 의념 피해 1회당 최종+4% (중첩)
  };
}
...
function sumBuffAtk(state, opts) {
  ...
  for (const b of state.buffs) {
    if (b.endT <= state.t || !b.atk) continue;
    if (!_passSystemFilter(b, opts)) continue;
    s += b.atk * (b.stackCount || 1);
  }
  // 현염법체 기본(화 2+): 작열 상태 대상에게 신통 시전 시 공격력 +9% — 본체 신통 전용
  if (!(opts && opts._isLawDamage) && (state.catSlots.화염 || 0) >= 2 && (state.stacks.작열 || 0) > 0) s += 9;
  return s;
}
// isShintong=true: 신통 본체 피해에서 호출 (신통 전용 buff도 포함)
// isShintong=false: 천뢰/낙뢰/작열/호무/평타 등 — 신통 전용 buff 제외
function sumBuffCR(state, isShintong = true, opts) {
  let s = 0;
  const isTreasure = !!(opts && opts.type === '법보절대');
  for (const b of state.buffs) {
    if (b.endT <= state.t || !b.cr) continue;
    if (b.shintongOnly && !isShintong) continue;
    // treasureOnly: 법보 type 만 적용 (e.g., 유리옥호 법보 cr +30%)
    if (b.treasureOnly && !isTreasure) continue;
    if (!_passSystemFilter(b, opts)) continue;
    s += b.cr * (b.stackCount || 1);
  }
  // 뇌인 스택 / 공명뇌전2 — 본체 신통 전용 (법상 damage 제외)
  if (!(opts && opts._isLawDamage)) {
...
function sumBuffCD(state, isShintong = true, opts) {
  let s = 0;
  const isTreasure = !!(opts && opts.type === '법보절대');
  for (const b of state.buffs) {
...
function sumShintongInc(state) {
  let s = 0;
  for (const b of state.buffs) {
    if (b.endT > state.t && b.cat === 'inc' && b.dmgMult) {
      // attr 필터 — buff에 attr 가 지정되어 있으면 별도 sumBuffAttrInc 에서 처리 (sumShintongInc 에서는 제외)
      if (b.attr) continue;
      s += b.dmgMult * (b.stackCount || 1);
    }
  }
  // 방어법보 [명공현주] passive: 파괴되지 않은 동안 신통 피해 +8%
...
// 속성별 피해 증가 (물리/술법) — buff.attr 매칭 시 합산
// scope: 모든 피해 (신통/법보/호무/평타 등 — 단 attr 일치할 때만)
function sumBuffAttrInc(state, attr) {
  if (!attr) return 0;
  let s = 0;
  for (const b of state.buffs) {
    if (b.endT > state.t && b.cat === 'inc' && b.dmgMult && b.attr === attr) {
      s += b.dmgMult * (b.stackCount || 1);
    }
  }
  return s;
...
// 피해 속성 결정 (물리 / 술법 / 확정) — 신통_정리.md 원문 spec 기준
// 확정 (살혼 등) 은 attr 미반환 (세트 보너스 X)
// type → attr 매핑 우선, 그 다음 SK[skill].attr (신통별 명시)
// ※ family 기반 default 매핑은 사용 안 함 — 같은 family 안에서도 신통마다 attr 다름
//   예: 균천·진악(술법) vs 균천·현봉(물리), 청명·투진(술법) vs 청명·풍뢰(물리)
function getDamageAttr(state, type, opts) {
  ...
  if (type === '법보절대') {
    // 법보별 attr lookup (TREASURES 데이터의 attr 필드)
    const tr = state._currentTreasure && TREASURES[state._currentTreasure];
    return (tr && tr.attr) || null;
  }
  // 신통 (type === '신통') — SK[skill].attr 명시값 사용 (신통_정리.md 원문 기준)
  const skName = state._activeCast || state._currentSkill;
  if (skName && SK[skName] && SK[skName].attr) return SK[skName].attr;
  return null;  // 미분류 시 세트 보너스 미적용
}
...
function sumBuffDealt(state, isShintong = true, opts) {
  let s = 0;
  const isTreasure = !!(opts && opts.type === '법보절대');
  for (const b of state.buffs) {
    if (b.endT <= state.t || b.cat !== 'dealt' || !b.dmgMult) continue;
    if (b.shintongOnly && !isShintong) continue;
    if (b.treasureOnly && !isTreasure) continue;
    if (!_passSystemFilter(b, opts)) continue;
    s += b.dmgMult * (b.stackCount || 1);
...
function critMult(cr, cd) {
  // 기댓값 모드: cMult = (1 - cr%) × 1배 + cr% × 크리배수
  // cd 는 `치명타 배율` — 절대 배수 (cd=150% → 크리 = 일반의 1.5배)
  // 사양: 치명타 배율 100% 일 때 크리/일반 동일 (메리트 X), 150% 일 때 크리 1.5배
  cr = Math.max(0, Math.min(100, cr));
  return (100 - cr) / 100 * 1 + cr / 100 * (cd / 100);
}
...
function dealDamage(state, base, opts = {}) {
  ...
  //   소스: nextCast.finalDmg (다음 신통 — 현미/풍세) [신통 전용]
  //         + localFinalDmg (본 cast — 통백 / 환음요탑 / 참원선검 등) [모든 type 적용]
  //         + 유뢰4법체(crit) / 현염4법체(작열) [신통 전용]
  // 분신: nextCast 와 유뢰/현염 법체 효과는 미공유 (본체 stack 기반). localFinalDmg 는 cast 가 부여 → 분신도 적용
  let finalPct = 0;
...
function tickHitCounters(s, n) {
  ...
  if (s.검영End > s.t && s.famSlots.중광) {
    ...
    while (s.검영히트 >= 5 && s.검영발동 < 20) {
      ...
      record(s, dealDamage(s, 8, { noSkillMult: true, type: '호무', attr: '물리' }));
      ...
      if (s.파천카운터 >= 3) {
        ...
        record(s, dealDamage(s, 96, { noSkillMult: true, type: '호무', attr: '물리' }));
        s._currentSource = p;
      }
    }
  }
  ...
  if (s.열천End > s.t && s.famSlots.사해) {
    ...
    while (s.열천히트 >= 10 && s.열천발동 < 10) {
      ...
      if (s.마념카운터 >= 2) {
        ...
        record(s, dealDamage(s, 24, { noSkillMult: true, attr: '술법' }));
        s._currentSource = p;
      }
    }
  }
...
function detailedBuffBreakdown(state, bd) {
  ...
  // nextCast.inc (소비된 — 다음 신통 1회 신통피해 +N%, e.g., 옥추·소명 [성류] 옥추4+)
  // 신통 type 일 때만 표시 (실제 적용도 isShintong일 때만 됨)
  if (bd?.type === '신통' && bd?.ncInc) {
    const srcs = (state._consumedNextCastSources || []).filter(s => s.field === 'inc' && s.pct);
    if (srcs.length > 0) for (const s of srcs) cat.신통피해.push(`${s.key}+${s.pct.toFixed(0)}`);
    else cat.신통피해.push(`nextCast+${bd.ncInc.toFixed(0)}`);
  }
  ...
  if (bd?.type === '신통') {
    if (bd?.finalDmgPct) {
      // 소비된 nextCast source 가 있으면 옵션명별로 분리해서 표시 (예: 복룡·약영 → 현미+45)
      // 없으면 fallback 으로 'nextCast+합계' 표시 (구버전 호환)
      const srcs = state._consumedNextCastSources || [];
      const finalSrcs = srcs.filter(s => (s.field || 'finalDmg') === 'finalDmg' && s.pct);
      if (finalSrcs.length > 0) {
        for (const s of finalSrcs) cat.최종피해.push(`${s.key}+${s.pct.toFixed(0)}`);
      } else {
        cat.최종피해.push(`nextCast+${bd.finalDmgPct.toFixed(0)}`);
      }
    }
...
function record(state, amount, source) {
  ...
  // 시간별 누적 피해 트래킹 (totalDmg / dmgEvents push 는 호신강기/HP 풀 적용 후 displayDmg 로 처리)
  ...
  // === 자기 비술: 분혼 허 — 봉인 종료 후 호신강기 피해 심화 +18% (10초) ===
  // (악신 분신은 위 dealDamage 에서 별도 계산되어 state._cloneDmgPending 에 저장됨 — 아래에서 별도 emit)
  // 사양: "호신강기에 대한 피해 심화 +18%" → amp% 카테고리 합산. 호신강기 hit 부분에만 적용.
  // 처리는 호신강기 분배 로직 (아래 line ~1500) 안에서 amount × shieldRatio 로 확장 후 HP overflow 부분만 원본 amount 기준으로 환원.
  ...
  // === 호신강기/HP 풀 적용 ===
  // 호신강기/HP 모두 defMult (방어 + defDebuff) 적용된 amount 그대로 사용
  // (사용자 요청: 방어막에도 데미지 감소 동일 적용 — 본 피해 = 호신강기 차감 = HP 차감)
  const bd = state._lastBreakdown;
  // 자기 비술 분혼 봉인 active 동안 → 호신강기 수치 없는 것처럼 취급 (사용자 정의)
  //   - hit 시 호신강기 우회 (bypassShield: HP 직접 차감)
  //   - shield rem / destroyed 는 변경 X (봉인 종료 후 원래 수치로 복귀)
  //   - 방어법보 자체는 살아있어 옵션/buff 정상 동작 (set 보너스, 천창홍고 max+15%, applyBuff 영구 active 등)
  const 봉인active = (state.봉인EndT || 0) > state.t;
  const bypassShield = 봉인active || isBypassShield(state, bd);
  ...
  // === 표시 피해: 실제 풀에 차감된 합 (호신강기 raw + HP post-defMult) ===
  // bypassShield → hpHit (raw 그대로)
  // 그 외 → shieldHit + hpHit (호신강기 raw + HP post-defMult)
  const displayDmg = bypassShield ? hpHit : (shieldHit + hpHit);
  state.totalDmg = (state.totalDmg || 0) + displayDmg;
  state.dmgEvents.push({ t: state.t, amt: displayDmg, src, activeCast });
  // === 적 비술 발동 hook (HP 0 도달 등) ===
  // ※ 혼원마주는 별도 훅 — 방어법보 파괴 transition 시점에 발동 (방어법보_파괴효과 에서 처리)
  const enemyBisul = (CFG.bisul && CFG.bisul.enemy) || [];
  for (const b of enemyBisul) {
    if (!b || !b.master || !b.branch) continue;
    // 탁천: 치명일격 (HP 0 임계 도달) 시 — 적 HP 0 도달 시 1회
    // 식혼: "공격 후 사망 시" — sim 의 적 (target) HP 0 도달 시 1회
    // 두 비술 모두 적 HP 0 도달 시 1회 발동 (자기 데미지로 끝낼 때)
    if ((b.master === '탁천' || b.master === '식혼') && (state.hpRem || 0) <= 0 && !state['_' + b.master + '_fired']) {
      state['_' + b.master + '_fired'] = true;
      비술_발동_적(state, b.master, b.branch);
    }
  }
  // === 자기 탁천 발동 hook — 사양 "치명 일격을 받았을 때 (= HP 0 도달 시점)"
  // mirror 가정: 자기 ↔ 적 동일 → 적 HP 0 도달 = 자기 치명일격 시점
  // 흐름 (사용자 정의):
  //   1. 마스터 발동 시도 (CD 170s 검사). 발동 시 ① applyBuff + ② 즉시 1회 광역 + 차회광역 buff 생성 (비술_발동_자기 안에서 처리).
  //   2. 마스터 발동 X (CD 미경과) + 차회광역 buff 있으면 → 광역 1회 emit + buff 소진 (다음 HP 0 시 처리).
  //   → 한 cycle 당 총 2회 (마스터 발동 시점 1회 + 다음 HP 0 시 1회)
  if (!state._탁천허광역_처리중 && (state.hpRem || 0) <= 0) {
    state._탁천허광역_처리중 = true;
    const selfBisulTakcheon = (CFG.bisul && CFG.bisul.self) || [];
    let 마스터발동됨 = false;
    for (const b of selfBisulTakcheon) {
      if (!b || b.master !== '탁천' || !b.branch) continue;
      const beforeT = state.bisul_lastFireT && state.bisul_lastFireT['탁천'];
      비술_발동_자기(state, b.master, b.branch);
      const afterT = state.bisul_lastFireT && state.bisul_lastFireT['탁천'];
      if (beforeT !== afterT) 마스터발동됨 = true;
    }
    // 마스터 발동 X (CD 안) + 차회광역 buff 있으면 사용
    if (!마스터발동됨 && state.탁천허_차회광역) {
      state.탁천허_차회광역 = false;
      const hpDmg2 = (CFG.baseHP || 0) * 0.09;
      const atkCap2 = (CFG.baseATK || 0) * 16.20;
      const dmg2 = Math.min(hpDmg2, atkCap2);
      const prevSrc2 = state._currentSource;
      state._currentSource = `탁천마주·허·광역(2/2)`;
      TRACE(state, 'OPT', `🔮탁천마주·허 ② 차회광역 buff 발동 (HP 0 재도달, 2/2): ${(dmg2/1e8).toFixed(2)}억 = min(maxHP×9%, atk×1620%) [buff 소진]`);
      record(state, dmg2, `탁천마주·허·광역(2/2)`);
      state._currentSource = prevSrc2;
    }
    state._탁천허광역_처리중 = false;
  }
  ...
  if (state._cloneDmgPending && state._cloneDmgPending > 0) {
    ...
    // 호신강기/HP 분배
    let cloneShield = 0, cloneHp = 0;
    if (bypassShield) {
      cloneHp = cloneAmt;
      state.hpRem = Math.max(0, (state.hpRem || 0) - cloneHp);
    } else {
      // 악신·진 호신강기 심화 +33% — amp 합산 (호신강기 부분에만 적용, HP overflow 는 원본 amount 단위)
      let cloneShieldRatio = 1;
      let cloneAmountForShield = cloneAmt;
      if (state.악신_호신심화 > 0 && (state.shieldRem || 0) > 0 && cloneBd) {
        const ampPct = cloneBd.ampPct || 0;
        cloneShieldRatio = (100 + ampPct + state.악신_호신심화) / (100 + ampPct);
        cloneAmountForShield = cloneAmt * cloneShieldRatio;
      }
      const cloneShieldRes = applyShieldDamage(state, cloneAmountForShield, { type: cloneBd && cloneBd.type });
      cloneShield = cloneShieldRes.absorbed;
      const cloneOverflow = cloneShieldRes.remaining;
      if (cloneOverflow > 0) {
        cloneHp = cloneOverflow / cloneShieldRatio;
        state.hpRem = Math.max(0, (state.hpRem || 0) - cloneHp);
      }
    }
...
// ---------- 영검: 복룡 (저체력 특화) ----------
SK['복룡·절화'] = {
  fam: '복룡', cat: '영검', main: 135, attr: '술법',
  cast(s, slots) {
...
SK['복룡·약영'] = {
  ...
  cast(s, slots) {
    ...
    if (현미발동) {
      TRACE(s, 'OPT', `🟠약영·현미 발동: HP ${hpPct}% ≤ 60% → 다음 신통 최종피해 +45%`);
      // BUF trace — buff lane 에 표시 (다음 신통 cast 까지 무한 TTL)
      TRACE(s, 'BUF', `🔼버프 [복룡·약영 → 현미] 다음 신통 최종피해 +45%`);
      addNextCast(s, 'finalDmg', 45);
      pushNextCastSource(s, { key: '약영·현미', pct: 45, field: 'finalDmg', msg: `HP ${hpPct}% ≤ 60% → 다음 신통 최종피해 +45%` });
    }
  }
};
SK['복룡·결운'] = {
  fam: '복룡', cat: '영검', main: 172, attr: '술법',
  cast(s, slots) {
    const hpPct = (hpRatio(s) * 100).toFixed(0);
...
function 천검발동(s, slots, ampPct = 0, srcTag = '천검') {
  ...
  // HP 100%→60%, HP 0%→120%, 실시간 HP 기반 선형 보간 (호신강기는 무시 — HP 만 기준)
...
SK['균천·진악'] = {
  fam: '균천', cat: '영검', main: 225, attr: '술법',
  cast(s, slots) {
    ...
    for (let i = 0; i < 진악Cnt; i++) {
      record(s, dealDamage(s, 30, { noSkillMult: true, type: '호무', attr: '술법' }), `진악(호무) ${i+1}/${진악Cnt}`);
    }
    // [동현] 50% 호무 × 2회 (max tier)
    TRACE(s, 'OPT', `🟠진악·동현 발동: 50% 호무 ×2`);
    for (let i = 0; i < 2; i++) {
      record(s, dealDamage(s, 50, { noSkillMult: true, type: '호무', attr: '술법' }), `동현(호무) ${i+1}/2`);
    }
    // [절학] 검세 5+ 시 105% 호무 × 2회 (max tier)
    if (js >= 5) {
      TRACE(s, 'OPT', `🟠진악·절학 발동: 검세 ${js}중첩 ≥ 5 → 105% 호무 ×2`);
      for (let i = 0; i < 2; i++) {
        record(s, dealDamage(s, 105, { noSkillMult: true, type: '호무', attr: '술법' }), `절학(호무) ${i+1}/2`);
      }
    }
    // === 본 신통 (술법 일반 피해) ===
...
SK['균천·현봉'] = {
  fam: '균천', cat: '영검', main: 252, attr: '물리',
  cast(s, slots) {
    const js = s.stacks.검세;
    // === "본 신통 시전 시" 트리거 (본 신통 record 전) ===
    ...
    if (js >= 3) {
      ...
      record(s, dealDamage(s, 140, { noSkillMult: true, type: '호무', attr: '물리' }), '새벽빛(호무)');
    }
    ...
    record(s, dealDamage(s, 60, { noSkillMult: true, type: '호무', attr: '물리' }), '절진(호무)'); // 60% 호무 추가
  }
};
SK['균천·파월'] = {
  fam: '균천', cat: '영검', main: 225, attr: '술법',
  cast(s, slots) {
    ...
    record(s, dealDamage(s, 100, { noSkillMult: true, type: '호무', attr: '술법' }), '여명(호무)');
    검세획득_균천(s, slots, 1);
    // === 본 신통 ===
    record(s, dealDamage(s, 225));
    // === 본 신통 명중 후 ===
    ...
    record(s, dealDamage(s, 60, { noSkillMult: true, type: '호무', attr: '술법' }), '귀진(호무)'); // 60% 호무 추가
  }
};
SK['균천·관일'] = {
  fam: '균천', cat: '영검', main: 250, attr: '물리',
  cast(s, slots) {
    ...
    record(s, dealDamage(s, 40, { noSkillMult: true, type: '호무', attr: '물리' }));
    s._currentSource = prevSrc;
    if (s.famSlots.균천) 검세획득_균천(s, s.famSlots.균천, 1);
    s._관일이미처리 = true;
  }
...
SK['참허·단진'] = {
  fam: '참허', cat: '영검', main: 200, attr: '물리',
  cast(s, slots) {
    ...
    record(s, dealDamage(s, 40, { noSkillMult: true, type: '호무', attr: '물리' }), '참파(호무)'); // 40% 호무 추가
  }
};
SK['참허·엄동'] = {
  fam: '참허', cat: '영검', main: 180, attr: '술법',
  cast(s, slots) {
    검심획득(s, 1);
    const cm = s.stacks.검심통명 ? 1 : 0;
    ...
    record(s, dealDamage(s, 60, { noSkillMult: true, type: '호무', attr: '술법' }), '엄동(호무)');
    if (cm) record(s, dealDamage(s, 60, { noSkillMult: true, type: '호무', attr: '술법' }), '엄동·통명(호무)');
  }
};
SK['참허·분광'] = {
  fam: '참허', cat: '영검', main: 180, attr: '물리',
  cast(s, slots) {
    ...
    record(s, dealDamage(s, 24, { noSkillMult: true, type: '호무', attr: '물리' }));
    s._currentSource = prevSrc;
    s._분광이미처리 = true;
    // 본 신통
    record(s, dealDamage(s, 180));
    // [참공] 80% 호무 추가 1회
    record(s, dealDamage(s, 80, { noSkillMult: true, type: '호무', attr: '물리' }), '참공(호무)');
  }
};
...
SK['중광·귀사'] = {
  fam: '중광', cat: '영검', main: 150, attr: '물리',
  cast(s, slots) {
    ...
    if (hpBelow(s, 0.60)) record(s, dealDamage(s, 160, { noSkillMult: true, type: '호무', attr: '물리' }), '관일(호무)');
  }
};
SK['중광·투영'] = {
  fam: '중광', cat: '영검', main: 152, attr: '물리',
  cast(s, slots) {
    ...
    if (hpBelow(s, 0.60)) record(s, dealDamage(s, 18 * 20, { noSkillMult: true, type: '호무', attr: '물리' }), '검홍(호무)');
  }
};
SK['중광·육요'] = {
  fam: '중광', cat: '영검', main: 150, attr: '물리',
...
SK['중광·환성'] = {
  fam: '중광', cat: '영검', main: 128, attr: '물리',
  cast(s, slots) {
...
function 염양발동(s, slots) {
  ...
  record(s, dealDamage(s, total, { noSkillMult: true, attr: '술법' }), '염양(유파)');
  ...
  if (s.selectedSkills && s.selectedSkills.has('열산·순일')) {
    ...
    if ((s.순일남은 || 0) > 0) {
      ...
      s.순일남은--;
      record(s, dealDamage(s, 40 + 20, { noSkillMult: true, attr: '물리' }), '순일');
    }
  }
  ...
  if (s.selectedSkills && s.selectedSkills.has('열산·양운')) {
    ...
    if ((s.진염남은 || 0) > 0) {
      ...
      s.진염남은--;
      record(s, dealDamage(s, 60, { noSkillMult: true, attr: '물리' }), '진염');
    }
  }
...
SK['열산·염폭'] = {
  fam: '열산', cat: '화염', main: 225, attr: '술법',
  cast(s, slots) {
    ...
    if (bonus) record(s, dealDamage(s, bonus, { noSkillMult: true, attr: '술법' }), '진연');
...
SK['열산·양운'] = {
  fam: '열산', cat: '화염', main: 212, attr: '물리',
  cast(s, slots) {
...
SK['열산·성료'] = {
  fam: '열산', cat: '화염', main: 213, attr: '술법',
  cast(s, slots) {
    const startLaysan = 열산상태(s);
    ...
    if (extra) record(s, dealDamage(s, extra, { noSkillMult: true, attr: '술법' }), '치운');
    // === DMG 후 작열 부여 ===
...
SK['열산·순일'] = {
  fam: '열산', cat: '화염', main: 225, attr: '물리',
  cast(s, slots) {
...
function 폭파(s) {
  ...
  if (s.selectedSkills && s.selectedSkills.has('형혹·함양') && s.함양End > s.t) {
    if ((s.함양남은 || 0) > 0) {
      ...
      record(s, dealDamage(s, 24, { noSkillMult: true, attr: '술법' }), '함양');
      record(s, dealDamage(s, 20, { noSkillMult: true, attr: '술법' }), '염화');
    }
  }
}
...
SK['형혹·업화'] = {
  fam: '형혹', cat: '화염', main: 170, attr: '술법',
  cast(s, slots) {
...
SK['형혹·겁염'] = {
  fam: '형혹', cat: '화염', main: 170, attr: '물리',
  cast(s, slots) {
...
SK['이화·풍권'] = {
  fam: '이화', cat: '화염', main: 135, attr: '술법',
  cast(s, slots) {
...
SK['이화·염우'] = {
  fam: '이화', cat: '화염', main: 128, attr: '술법',
  cast(s, slots) {
...
SK['천로·직염'] = {
  ...
  cast(s, slots) {
    ...
    if (remainingDot > 0) {
      // [열기] 직전 직염이 부여한 펜딩이 있으면 작열폭발 피해 +40% (additive — 피해 증가, 최종 아님)
      const 열기보너스 = s._열기pending || 0;
      // 이미 atk/def 반영된 스냅샷 피해이므로 bare record (× 1.3 성화 증폭 + 유파 slot×15 수동 적용 + 열기 보너스)
      const popDmg = remainingDot * 1.3 * (1 + slots * 15 / 100) * (1 + 열기보너스 / 100);
      record(s, popDmg, 열기보너스 > 0 ? `성화(폭발) +열기${열기보너스}%` : '성화(폭발)');
      s._열기pending = 0; // 소비
    }
    // [열기] 다음번 작열 폭발 피해 +40% (max tier) — 다음 직염의 폭발에 적용
    TRACE(s, 'OPT', `🟠직염·열기 발동: 다음 작열 폭발 피해 +40%`);
    s._열기pending = 40;
  }
};
SK['천로·유형'] = {
...
SK['천로·운화'] = {
  fam: '천로', cat: '화염', main: 135, attr: '물리',
  cast(s, slots) {
    prune작열(s);
...
SK['청명·천노'] = {
  fam: '청명', cat: '뇌전', main: 225, attr: '물리',
  cast(s, slots) {
...
SK['청명·붕운'] = {
  fam: '청명', cat: '뇌전', main: 225, attr: '술법',
  cast(s, slots) {
    ...
    addNextCast(s, 'finalCD', 25);
    pushNextCastSource(s, { key: '붕운·파정', pct: 25, field: 'finalCR', msg: '다음 신통 최종 치명타율 +25%' });
    pushNextCastSource(s, { key: '붕운·파정', pct: 25, field: 'finalCD', msg: '다음 신통 최종 치명타 배율 +25%' });
  }
};
SK['청명·풍뢰'] = {
  fam: '청명', cat: '뇌전', main: 225, attr: '물리',
...
SK['옥추·황룡'] = {
  fam: '옥추', cat: '뇌전', main: 172, attr: '술법',
  cast(s, slots) {
...
SK['옥추·소명'] = {
  fam: '옥추', cat: '뇌전', main: 170, attr: '물리',
  cast(s, slots) {
...
SK['옥추·수광'] = {
  ...
  cast(s, slots) {
    ...
    옥추획득(s);
    record(s, dealDamage(s, 30, { noSkillMult: true, attr: '물리' }), '수광(지속)');
    s._수광이미처리 = true;
...
SK['옥추·청사'] = {
  fam: '옥추', cat: '뇌전', main: 170, attr: '술법',
  cast(s, slots) {
    ...
    if (옥추Stack >= 2) record(s, dealDamage(s, 60 * 옥추유파Mult(s, slots), { noSkillMult: true, attr: '술법' }), '청사(옥추2+)');
  }
};
...
SK['오뢰·천강'] = {
  fam: '오뢰', cat: '뇌전', main: 128, attr: '술법',
  cast(s, slots) {
...
SK['오뢰·호후'] = {
  fam: '오뢰', cat: '뇌전', main: 135, attr: '술법',
  cast(s, slots) {
...
SK['신소·운록'] = {
  fam: '신소', cat: '뇌전', main: 135, attr: '물리',
  cast(s, slots) {
...
SK['신소·천고'] = {
  ...
  cast(s, slots) {
    ...
    if (CFG.randomCrit) {
      ...
      if (anyCrit) record(s, dealDamage(s, 165, { noSkillMult: true, attr: '물리' }));
    } else {
      const p만균 = 1 - Math.pow(1 - crEff, 3);
      if (p만균 > 0) record(s, dealDamage(s, 165 * p만균, { noSkillMult: true, attr: '물리' }));
    }
  }
};
SK['신소·환뢰'] = {
  fam: '신소', cat: '뇌전', main: 128, attr: '물리',
  cast(s, slots) {
    신소획득(s, 1);
    ...
    TRACE(s, 'BUF', `🔼버프 [신소·환뢰 → 풍세] 다음 신통 최종피해 +20%`);
    addNextCast(s, 'finalDmg', 20);
    pushNextCastSource(s, { key: '환뢰·풍세', pct: 20, field: 'finalDmg', msg: '다음 신통 최종피해 +20%' });
  }
};
SK['신소·청삭'] = {
  fam: '신소', cat: '뇌전', main: 128, attr: '물리',
  cast(s, slots) {
    ...
    if (천위) record(s, dealDamage(s, 천위, { noSkillMult: true, attr: '물리' }), '천위');
  }
};
...
function 명화살혼발사(s, pct) {
  ...
  if (s.유령불카운터 >= 2) {
    ...
    record(s, dealDamage(s, 30, { noSkillMult: true, attr: '물리' }));
    s._currentSource = prev;
  }
}
...
function 현화트리거(s) {
  ...
  record(s, dealDamage(s, 40, { noSkillMult: true, attr: '술법' }));
  s._currentSource = prevSrc;
}
...
function 제율트리거(s, frac = 1) {
  ...
  record(s, dealDamage(s, (30 + 30) * eff, { noSkillMult: true, attr: '술법' }));
...
function 살혼발사(s, overridePct) {
  ...
  if (s.폭우End > 0 && s.t < s.폭우End) {
    ...
    if (s.폭우살혼 % 3 === 0 && s.폭우발동 < 5) {
      s.폭우발동++;
      record(s, dealDamage(s, 30, { noSkillMult: true, attr: '물리' }), '폭우(발동)');
...
function onGyeokbal(s, frac = 1) {
  ...
  if (sel.has('주술·경선')) {
    ...
    if (eff > 0) {
      s.경선발동 = start + eff;
      record(s, dealDamage(s, (40 + 20) * eff, { noSkillMult: true, attr: '술법' }), '경선(격발)');
    }
  }
...
// 주술 (4)
SK['주술·제율'] = {
  fam: '주술', cat: '백족', main: 300, attr: '술법',
  cast(s, slots) {
...
SK['주술·태사'] = {
  fam: '주술', cat: '백족', main: 270, attr: '물리',
  cast(s, slots) {
    const 합2 = 계약합(s);
    ...
    if (합2 >= 2) {
      ...
      record(s, dealDamage(s, 100, { noSkillMult: true, attr: '물리' }), '독고저주');
      독고부여(s, 2);
    }
    ...
    if (합2 > 0) {
      ...
      record(s, dealDamage(s, 30 * cnt, { noSkillMult: true, attr: '물리' }), '겁인');
    }
    // [망식] 계약 3중첩 이상 시 180% 물리 1회 (max tier)
    if (합2 >= 3) {
      s._currentSource = '망식(계약3+)';
      TRACE(s, 'OPT', `🟠망식 발동: 계약 ${합2}중첩 → 180% 물리`);
      record(s, dealDamage(s, 180, { noSkillMult: true, attr: '물리' }), '망식');
    }
    s._currentSource = prev;
  }
};
SK['주술·경선'] = {
  fam: '주술', cat: '백족', main: 270, attr: '술법',
  cast(s, slots) {
    s.경선발동 = 0;
    s.원한발동 = 0;
    // 본 신통 main — "3명 2회 공격, 총 270% 술법" 멀티히트 균등 분배
    recordEvenHit(s, 270, 2);
    // [심장] 1~3 독고 부여 + 40% 술법 × 평균 2회 (max tier)
    독고부여(s, 2);
    const prev = s._currentSource;
    s._currentSource = '심장(평균2회)';
    record(s, dealDamage(s, 40 * 2, { noSkillMult: true, attr: '술법' }), '심장');
    s._currentSource = prev;
  }
};
SK['주술·유식'] = {
  fam: '주술', cat: '백족', main: 300, attr: '물리',
  cast(s, slots) {
    // [유식] 단진 패턴 — 카운터 reset 은 main loop pre-cast hook 에서 처리됨
    // 본 신통 main — "지정 4회 공격, 총 300% 물리" 멀티히트 균등 분배
    recordEvenHit(s, 300, 4);
    const prev = s._currentSource;
    // [심장] 1~3 독고 + 40% × 평균 2회 물리 (max tier) — 본 신통 시전 시 즉발
    독고부여(s, 2);
    s._currentSource = '심장(평균2회)';
    record(s, dealDamage(s, 40 * 2, { noSkillMult: true, attr: '물리' }), '심장');
...
// 사해 (4)
SK['사해·열천'] = {
  fam: '사해', cat: '백족', main: 187, attr: '술법',
  cast(s, slots) {
    ...
    살혼발사(s, 30);
    살혼발사(s, 30);
    // [천살] 본 신통 피해 +15% (max tier) — dealtPct 카테고리로 추가 (보너스 base 에 영향 없도록 localDealt 사용)
    record(s, dealDamage(s, 187, { localDealt: 15 }));
  }
};
SK['사해·폭우'] = {
  fam: '사해', cat: '백족', main: 220, attr: '물리',
  cast(s, slots) {
    ...
    살혼발사(s, 30);
    살혼발사(s, 30);
    // 본 신통 DMG — "지정한 적을 5회 공격, 총 220% 물리" 멀티히트 균등 분배
    recordEvenHit(s, 220, 5);
  }
};
SK['사해·업련'] = {
  fam: '사해', cat: '백족', main: 187, attr: '술법',
...
SK['사해·명화'] = {
  fam: '사해', cat: '백족', main: 198, attr: '물리',
  cast(s, slots) {
    // [2마상] 시전 시마다 5회로 리셋
    s.마상남은 = 5;
...
// ======================== 비술 (秘術) ========================
// 6 마주 × 3 갈래 (무/허/진). 정확히 3개 장착.
// self 측 (자기 사용) + enemy 측 (상대 사용 — 적 effective HP/방어 영향) 분리.
// CFG.bisul = { self: [{ master, branch }, ...], enemy: [...] }  (각 max 3개)

// 비술 메타정보 — UI/spec 참조용
const BISUL_MASTERS = {
  탁천: { name: '탁천마주', trigger: '치명일격 (HP 0 도달)', cd: 170 },
  분혼: { name: '분혼마주', trigger: '신통/법보 첫 cast', cd: 160 },
  식혼: { name: '식혼마주', trigger: '신통/법보로 공격 후 (지속: 사망 때까지, 흡수 최대 1개)', cd: 160 },
  악신: { name: '악신마주', trigger: '방어법보 파괴 (호신강기 0 또는 4회 공격)', cd: 170 },
  혼원: { name: '혼원마주', trigger: '방어법보 파괴 시 (정확 spec — 무 90% / 허 60% / 진 76% 평균값 기준 보호막 복구, 너프 적용 2026-05-09)', cd: 160 },
  업화: { name: '업화마주', trigger: '신통/법보 5회 공격마다', cd: 160 },
};

// 자기 비술 발동 (DPS 영향 있는 갈래만 sim 적용)
...
function 비술_발동_자기(state, master, branch) {
  // 비술별 재사용 시간 (사양 verbatim — 신통_정리.md):
  //   탁천 170s, 분혼 160s, 식혼 160s, 악신 170s, 혼원 160s, 업화 160s
  const BISUL_CD = (BISUL_MASTERS[master] && BISUL_MASTERS[master].cd) || 160;
  ...
  if (master === '분혼') {
    // 사양 (신통_정리.md:768/771/774):
    //   무: "15초간 대상의 방어법보를 봉인하며, 대상을 4회 공격하여 총 공격력 1000.00%의 피해"
    //   허: "10초간 대상의 방어법보를 봉인하며, 봉인 종료 후 10초간 18.00%의 호신강기에 대한 피해 심화. 추가로 4회 공격하여 총 공격력 1000.00%의 피해"
    //   진: "15초간 자신을 강화시키며, 호신강기에 입히는 피해의 25.00%만큼 추가로 대상의 생명력을 감소. 대상을 4회 공격하여 총 공격력 800.00%의 피해"
    // 봉인 정의 (사용자): 호신강기 수치가 없는 것으로 취급. 방어법보 destroyed X, 방어법보 buff 정상 동작.
    //   sim 구현: state.봉인EndT 설정 → record() 안에서 hit 가 호신강기 우회 (HP 직접 차감), shield rem 보존.
    const dur = branch === '진' ? 0 : (branch === '허' ? 10 : 15);
    if (dur > 0) state.봉인EndT = state.t + dur;
    // 사양: "대상을 4회 공격하여 총 공격력 N%" — 4 hit × (N/4)% (멀티히트)
    const totalDmg = branch === '진' ? 800 : 1000;
    const hitCount = 4;
    const perHit = totalDmg / hitCount; // 무/허 = 250%, 진 = 200%
    TRACE(state, 'OPT', `🔮분혼마주·${branch} 발동: ${dur}초 호신강기 봉인 + 총 ${totalDmg}% (${hitCount} hit × ${perHit}%)`);
    const prev = state._currentSource;
    for (let i = 0; i < hitCount; i++) {
      state._currentSource = `분혼마주·${branch}(${i + 1}/${hitCount})`;
      record(state, dealDamage(state, perHit, { noSkillMult: true, bypassShield: dur > 0 }));
    }
    ...
    // 사양 (신통_정리.md:791-798):
    //   무: "적의 모든 방어법보가 파괴되면 ... 분신 본체의 39.00%이며, 어떠한 피해도 받지 않는다."
    //   허: "자신의 모든 방어법보가 파괴되면 ... 분신 본체의 20.00%이며, 그중 생명력 속성은 150.00%까지 증가하지만 입는 피해를 받는다. 분신 지속 시간 동안 수련자가 받는 피해의 26.00%가 분신에게 전이된다."
    //   진: "4회 공격 후 ... 분신 본체의 30.00%이며, 호신강기에 대한 피해 심화를 33.00% 획득하고 분신은 어떠한 피해도 받지 않는다."
    // sim 적용: 본체 N% 속성 분신 데미지 + 진의 호신강기 심화 +33% (record() 분신 처리)
    // applyBuff 등록 — 허의 "생명력 150% / 26% 전이", 무/진의 "분신 무피격" 도 buff 등록 (sim dispatch 미사용)
    ...
    if (branch === '무') {
      ...
      applyBuff(state, '악신허_분신생명력', { 분신생명력배율: 1.5 }, 15);
      applyBuff(state, '악신허_분신피격', { 분신피격: true }, 15);
      applyBuff(state, '악신허_분신피해전이', { 자기피해전이: 26 }, 15);
    } else if (branch === '진') {
      applyBuff(state, '악신진_분신무피격', { 분신무피격: true }, 15);
    }
    const 허_extra = branch === '허' ? ' + 생명력 150% / 분신 입는 피해 받음 / 자기 받는 피해 26% 전이 (applyBuff 등록, sim 자기 HP/분신 HP 미모델 → 결과 영향 X)' : '';
    const 무진_extra = (branch === '무' || branch === '진') ? ' + 분신 어떠한 피해도 받지 않음 (applyBuff 등록)' : '';
    TRACE(state, 'OPT', `🔮악신마주·${branch} 발동: 15초간 분신 추가 데미지 (본체 ${pct}% 속성)${branch === '진' ? ' + 호신강기 심화 +33%' : ''}${허_extra}${무진_extra}`);
  } else if (master === '식혼') {
    // 사양 (신통_정리.md:780/783/786): "신통 또는 법보로 공격 후 사망할 때까지 자신의 방어법보를 1개 흡수한다(최대 1개). 140초간 모든 방어법보의 호신강기 총합의 N%에 해당하는 생명력 및 최대 생명력을 획득"
    //   "사망할 때까지" = 효과 지속 구간 (영구). 트리거는 "공격 후" → 첫 cast.
    //   흡수 대상: 마지막 순서 방어법보 (사용자 명시 — equipment 마지막 슬롯).
    //   mirror 모델: 자기 = 더미 동일 → state.shields[last].destroyed 처리.
    if (branch === '진') {
      // 사양: "치명타율이 9.00%~19.00% 증가하며, 자신의 현재 생명력 백분율이 낮을수록 치명타율이 증가한다(생명력 백분율이 25.00%일 시 최대값을 적용)"
      // sim 적용: cr buff (HP 비율 따라 9~19% 스케일)
      ...
      // 사양: "140초간 모든 방어법보의 호신강기 총합의 76.00%에 해당하는 생명력 및 최대 생명력을 획득"
      // applyBuff 등록 — sim dispatch 미사용 (자기 HP 변화 X) 이지만 buff 자체는 사양대로 active.
      const shieldSum = state.shields ? state.shields.reduce((s, sh) => s + sh.max, 0) : 0;
      const hpGain = shieldSum * 0.76;
      applyBuff(state, '식혼무_HP회복', { selfHpGain: hpGain, selfMaxHpGain: hpGain }, 140);
      TRACE(state, 'OPT', `🔮식혼마주·무 발동 (자기): 호신강기 총합 ${(shieldSum/1e8).toFixed(2)}억 × 76% = HP/max HP +${(hpGain/1e8).toFixed(2)}억 (140초, applyBuff 등록 — sim 자기 HP 미모델 → 결과 영향 X)`);
    } else if (branch === '허') {
      // 사양: "140초간 모든 방어법보의 호신강기 총합의 46.00%에 해당하는 생명력 및 최대 생명력을 획득. 지속 기간 동안 피해 감면이 7.00%~12.00% 증가 (HP 25% 시 최대)"
      // applyBuff 등록 — (1) HP 회복 (selfHpGain) (2) 피해 감면 +7~12% (자기 받는 피해 감면). 둘 다 sim dispatch 미사용.
      const shieldSum = state.shields ? state.shields.reduce((s, sh) => s + sh.max, 0) : 0;
      const hpGain = shieldSum * 0.46;
      const reductionVal = selfHpScale(state, 7, 12, 0.25);
      applyBuff(state, '식혼허_HP회복', { selfHpGain: hpGain, selfMaxHpGain: hpGain }, 140);
      applyBuff(state, '식혼허_자기감면', { 자기감면: reductionVal }, 140);
      TRACE(state, 'OPT', `🔮식혼마주·허 발동 (자기): 호신강기 총합 × 46% = HP/max HP +${(hpGain/1e8).toFixed(2)}억 + 자기 피해 감면 +${reductionVal.toFixed(1)}% (140초, applyBuff 등록 — sim 자기 HP/받는 피해 미모델 → 결과 영향 X)`);
    }
    // 자기 방어법보 1개 흡수 (mirror — 마지막 순서의 방어법보 destroy 처리)
    // 흡수 정의 (사용자 명시): "파괴와 비슷한 처리, 해당 기능이 동작을 안 함"
    //   - destroyed = true → 호신강기 hit 안 들어감, 옵션 비활성 (파괴와 동일 효과)
    //   - state.shieldRem 갱신에서 제외
    //   - 다른 시스템의 destroyed 검사 (모든방어법보파괴 등) 에서 흡수 = 파괴 카운트 → 악신·허/무 트리거 발동 가능
    ...
    if (branch === '진') {
      ...
      if (branch === '허') {
        // 사양 (신통_정리.md:822): "또한 10초간 대상의 피해 심화/감면을 11.00%만큼 추가 감소시킨다."
        // 두 디버프 모두 applyBuff 등록 — sim 결과 영향 여부와 무관하게 buff/debuff 자체는 사양대로 등록.
        // (1) "대상의 피해 심화 -11%" = 적의 amp stat (적이 입히는 피해 증가) -11%
        //     sim dispatch: 미사용 (sim 은 자기가 적에게 입히는 데미지만 추적, 적의 amp 는 자기 받는 피해에만 영향)
        //     → buff 등록만, sim 결과 영향 X
        applyBuff(state, '업화허_적심화감소', { 적amp: -11 }, 10);
        // (2) "대상의 피해 감면 -11%" = 적의 reduction stat (적이 받는 피해 감면) -11%
        //     sim dispatch: defDebuff 매핑 (defMult = 0.7 + 0.3 × (감소/100) → 적 데미지 통과율 증가)
        applyBuff(state, '업화허_적감면감소', { defDebuff: 11 }, 10);
        TRACE(state, 'OPT', `   └ 업화·허: 10초간 적 피해 심화 -11% (sim 자기 받는 피해 미모델 → 결과 영향 X) + 적 받는 피해 감면 -11% (defDebuff 매핑)`);
      }
    }
  } else if (master === '탁천') {
    // 자기 탁천: 사양 "치명 일격을 받았을 때 (= HP 0 도달 시점)" — 적 HP 0 도달 시 발동 (mirror 가정)
    // applyBuff 등록 — sim dispatch 미사용 (자기 HP/받는 피해 변화 X) 이지만 buff 자체는 사양대로 active.
    if (branch === '무') {
      // 사양: "최대 생명력의 29.00%만큼 회복하며, 1초 동안 모든 피해를 면역"
      const hpGain = (CFG.baseHP || 0) * 0.29;
      applyBuff(state, '탁천무_HP회복', { selfHpGain: hpGain }, 0.001);  // 즉시 회복 (지속시간 X, 사양 명시 X)
      applyBuff(state, '탁천무_면역', { 자기면역: true }, 1);  // 1초 면역
      TRACE(state, 'OPT', `🔮탁천마주·무 발동 (자기): max HP 29% (+${(hpGain/1e8).toFixed(2)}억) 회복 + 1초 면역 (applyBuff 등록 — sim 자기 HP/받는 피해 미모델 → 결과 영향 X)`);
    } else if (branch === '허') {
      // 사양: ① max HP 18% 회복 + 1초 면역 (sim 미모델) ② 주변 최대 3명 (1:1 sim → 1명) 에게 max HP 9% 확정 피해 (atk 1620% 상한), 최대 2회 발동
      // ① 자기 HP/받는 피해 미모델 → 효과 없음
      // ② 데미지 공식: min(maxHP × 9%, atk × 1620%). lifetime cap 2회 (state.탁천허_발동수)
      // ① 자기 HP 18% 회복 + 1초 면역 — applyBuff 등록 (sim dispatch 미사용)
      const hpGain허 = (CFG.baseHP || 0) * 0.18;
      applyBuff(state, '탁천허_HP회복', { selfHpGain: hpGain허 }, 0.001);
      applyBuff(state, '탁천허_면역', { 자기면역: true }, 1);
      // ② 광역 즉시 1회 발동 (마스터 발동 시점)
      const hpDmg광역 = (CFG.baseHP || 0) * 0.09;
      const atkCap광역 = (CFG.baseATK || 0) * 16.20;
      const dmg광역 = Math.min(hpDmg광역, atkCap광역);
      const prevSrc광역 = state._currentSource;
      state._currentSource = `탁천마주·허·광역(1/2)`;
      TRACE(state, 'OPT', `🔮탁천마주·허 발동 (자기): ① HP 18% (+${(hpGain허/1e8).toFixed(2)}억) 회복 + 1초 면역 (applyBuff 등록) / ② 광역 즉시 1회 ${(dmg광역/1e8).toFixed(2)}억 = min(maxHP×9%, atk×1620%) + 차회광역 buff 생성 (다음 HP 0 시 1회)`);
      record(state, dmg광역, `탁천마주·허·광역(1/2)`);
      state._currentSource = prevSrc광역;
      // ② 차회광역 buff — 다음 HP 0 도달 시 광역 1회 더 발동
      state.탁천허_차회광역 = true;
    } else if (branch === '진') {
      // 사양: "최대 생명력의 21.00%만큼 회복하며 1초 동안 모든 피해를 면역. 6초간 피해 감면을 24.00%만큼 증가시킨다"
      // applyBuff 등록 — (1) HP 21% 회복 (2) 1초 면역 (3) 6초 자기 피해 감면 +24%
      const hpGain진 = (CFG.baseHP || 0) * 0.21;
      applyBuff(state, '탁천진_HP회복', { selfHpGain: hpGain진 }, 0.001);
      applyBuff(state, '탁천진_면역', { 자기면역: true }, 1);
      applyBuff(state, '탁천진_자기감면', { 자기감면: 24 }, 6);
      TRACE(state, 'OPT', `🔮탁천마주·진 발동 (자기): max HP 21% (+${(hpGain진/1e8).toFixed(2)}억) 회복 + 1초 면역 + 6초 자기 피해 감면 +24% (applyBuff 등록 — sim 자기 HP/받는 피해 미모델 → 결과 영향 X)`);
    }
  }
  return true;
}
...
// 적 비술 발동 (자기 데미지에 영향)
// 탁천: 자기 데미지가 적 HP 임계 도달 시 적 HP 회복 (사망 위기)
// 식혼: 자기가 적 호신강기 다 깎으면 발동 (적 부활/회복)
// 혼원: 호신강기 1/3 도달마다 적 호신강기 회복
// 분혼/악신/업화: 적 측은 자기 HP 영향 → sim 미모델
function 비술_발동_적(state, master, branch) {
  if (master === '탁천' || master === '식혼') {
    // 적 HP 회복 (max HP %)
    // 식혼 사양: "흡수한 방어법보의 호신강기 max × ratio"
    // 흡수 대상: 마지막 순서 방어법보 (사용자 명시)
    const ratio = master === '탁천'
      ? (branch === '무' ? 29 : branch === '허' ? 18 : 21)
      : (branch === '무' ? 76 : branch === '허' ? 46 : 47);
    let recover;
    if (master === '식혼') {
      // mirror 모델에서 self 식혼 발동 시 _식혼_absorbedMax 가 저장됨 → 그 값 사용
      // 또는 fallback: state.shields[last].max
      let absorbedMax = state._식혼_absorbedMax || 0;
      if (!absorbedMax && state.shields && state.shields.length > 0) {
        absorbedMax = state.shields[state.shields.length - 1].max;
      }
      recover = absorbedMax * (ratio / 100);
    } else {
      // 탁천: max HP × ratio
      recover = CFG.baseHP * (ratio / 100);
    }
    state.hpRem = Math.min(CFG.baseHP * 1.5, (state.hpRem || 0) + recover);
    TRACE(state, 'OPT', `🔮${master}마주·${branch} (적): HP +${(recover/1e8).toFixed(2)}억 회복${master === '식혼' ? ' (마지막 순서 방어법보 max × '+ratio+'%)' : ''}`);
    if (master === '탁천' && branch === '진') state.적_피해감면EndT = state.t + 6;
    if (master === '식혼' && branch === '허') state.적_저체력피해감면 = true;
  } else if (master === '혼원') {
    // ※ 혼원마주는 방어법보_파괴효과() 에서 직접 처리됨 (방어법보 파괴 transition 시점)
    // 이 분기는 legacy fallback — 호신강기 1/3 임계 방식의 구 트리거 (현재 미사용)
    // 너프 사양 (2026-05-09): 무 90% / 허 60% / 진 76% — 평균 보호막값 기준
    if (!state.shields || state.shields.length === 0) return;
    const avgShield = state.shields.reduce((s, sh) => s + sh.max, 0) / state.shields.length;
    const pct = branch === '무' ? 90 : (branch === '허' ? 60 : 76);
    const recover = avgShield * (pct / 100);
    state.shieldRem = (state.shieldRem || 0) + recover;
    TRACE(state, 'OPT', `🔮혼원마주·${branch} (적, legacy): 호신강기 +${(recover/1e8).toFixed(2)}억 (평균 × ${pct}%)`);
    if (branch === '허') state.적_신통차단EndT = state.t + 12;
  }
}
...
function 비술_트리거검사(state) {
  ...
  for (const b of selfBisul) {
    ...
    // 악신·진: 사양 "신통 또는 법보로 4회 공격 후" — 누적 4회마다
    if (b.master === '악신' && b.branch === '진' && state._castCountTotal % 4 === 0) {
      비술_발동_자기(state, b.master, b.branch);
    }
    // 악신·허: 사양 "자신의 모든 방어법보가 파괴되면" — mirror 모델 (자기 = 더미 동일)
    if (b.master === '악신' && b.branch === '허' && 모든방어법보파괴(state)) {
      비술_발동_자기(state, b.master, b.branch);
    }
    // 악신·무: 사양 "적의 모든 방어법보가 파괴되면"
    if (b.master === '악신' && b.branch === '무' && 모든방어법보파괴(state)) {
      비술_발동_자기(state, b.master, b.branch);
    }
    // 업화: 진 = 매 cast 후 / 무·허 = 사양 "5회 공격할 때마다"
    if (b.master === '업화') {
      if (b.branch === '진') {
        비술_발동_자기(state, b.master, b.branch);
      } else {
...
function 영역_pendingFire(s) {
  ...
  // 명중 모델 — 발동 시점 (fireT) buff 가 적용. 다른 모든 데미지와 통일.
  const savedT = s.t;
  s.t = fireT;
  const prevSrc = s._currentSource;
  ...
  // AOE 900% — 1:1 sim 이라도 dummy 에게 적용 (광역 효과가 단일 대상 포함)
  s._currentSource = `영역(${name})·AOE`;
  record(s, dealDamage(s, 900, { type: '영역' }));
  s._currentSource = `영역(${name})`;
  // === 자기 버프 ===
  if (name === '진양화련') {
...
function 영역_시전트리거(s) {
  if (!CFG.영역) return;
  const name = CFG.영역;
  if (CFG.debugDealDamage) console.log(`[DBG][영역_시전트리거] t=${s.t.toFixed(2)} name=${name} 천위End=${(s.영역_천위End||0).toFixed(2)} 심연End=${(s.영역_심연End||0).toFixed(2)} 천위active=${s.t < (s.영역_천위End||0)} 심연active=${s.t < (s.영역_심연End||0)}`);
  if (name === '제왕의 정' && s.t < (s.영역_천위End || 0)) {
    // 천위: 70% × 2~4회 (기댓값 3회) = 210% (확정 피해 → noSkillMult, type='호무')
    const prevSrc = s._currentSource;
    s._currentSource = `영역(${name})·천위`;
    TRACE(s, 'OPT', `🌐영역 [천위] 트리거: 70% × 3회 (기댓값) = 210% 확정`);
    record(s, dealDamage(s, 210, { type: '영역' }));
    s._currentSource = prevSrc;
    ...
    const prevSrc = s._currentSource;
    s._currentSource = `영역(${name})·심연`;
    TRACE(s, 'OPT', `🌐영역 [심연] 트리거: 적 max HP 1.5% 또는 atk 300% (캡)`);
    record(s, dealDamage(s, 300, { type: '영역' }));
    s._currentSource = prevSrc;
  }
}
...
// - 빙의 종료 후 180초 CD → 다시 빙의 가능
// - 빙의 active 중에는 법상별 효과 발동 (per cast / 시간당 / 공격횟수당 등)
// - 빙의 시작/종료 시 cleanup 데미지 (각 법상마다 다름)
// 옵션: CFG.법상 = { name: '청교룡' | ... | null, tiers: { 실체:bool, 의념:bool, 진령:bool } }
...
function 법상_빙의active(s) {
  return s.법상_빙의시작T <= s.t && s.t < s.법상_빙의종료T;
}

// 매 cast 후 호출 — 빙의 트리거 + active 처리
function 법상_틱(s, opts) {
  if (!CFG.법상 || !CFG.법상.name || !법상_DEFS[CFG.법상.name]) return;
  ...
  if (s.t >= 다음빙의가능T && s.법상_빙의종료T <= s.t) {
    ...
    s.법상_금오피해카운터 = 0;
    s.법상_금오마지막발사T = -Infinity;
    s.법상_주작마지막발사T = -Infinity;
    s.법상_봉황마지막발사T = -Infinity;
    s.법상_진룡공격카운터 = 0;
    s.법상_진룡각인_누적 = 0;
    s.법상_적난새의념피해횟수 = 0;
...
// 적 상태 (defDebuff/crRes) 는 universal — 양쪽 모두 영향
function 법상Dmg(s, base, opts) {
  return dealDamage(s, base, { ...(opts || {}), noSkillMult: true, _isLawDamage: true });
}
...
function 법상_시작_cleanup(s, name, tiers) {
  ...
  if (name === '청반룡' && tiers.실체) {
    ...
    const prev = s._currentSource; s._currentSource = `법상·${name}·시작`;
  ...
  if (name === '봉황' && tiers.의념) {
    TRACE(s, 'OPT', `🦅봉황·의념 시작 cleanup: 봉황깃털 2개 (1개당 150% 확정) + 봉황 각인 +2`);
  ...
  if (name === '청교룡' && tiers.진령) {
    TRACE(s, 'OPT', `🐉청교룡·진령 시작 cleanup: 빙의 돌입 시 550% + 적 받는 최종피해 +20% (10초)`);
...
function 법상_매cast(s, name, tiers, castType) {
  ...
  if (name === '적난새' && tiers.실체) {
    ...
    // 기댓값 모드: 4개 평균 / 랜덤 모드: 3~5 중 random
    const 깃털수 = CFG.randomCrit ? (3 + Math.floor(Math.random() * 3)) : 4;
    const 진령배수 = tiers.진령 ? 1.5 : 1;
    TRACE(s, 'OPT', `🦅적난새·실체 발동: 신통/법보 공격 → 불깃털 ${깃털수}개${CFG.randomCrit ? ' (랜덤 3~5)' : ' (기댓값 4)'} (1개당 ${(3.5*진령배수).toFixed(2)}% 진원 확정)${tiers.진령 ? ' [진령 +50%]' : ''}`);
  ...
  if (name === '청반룡' && tiers.진령) {
    ...
    const prev = s._currentSource; s._currentSource = `법상·${name}·청린`;
    record(s, 법상Dmg(s, 220), `법상·${name}(진령)`);
    s._currentSource = prev;
  }
  if (name === '금오' && tiers.실체) {
  ...
  if (name === '진룡' && tiers.의념) {
    ...
    if (s.법상_진룡공격카운터 >= 2) {
      s.법상_진룡공격카운터 = 0;
      TRACE(s, 'OPT', `🐉진룡·의념 발동: 2회 cast 누적 → 청령 1개 (250%) + 진룡 각인 +1`);
      const prev = s._currentSource; s._currentSource = `법상·${name}·청령`;
...
function 법상_종료_cleanup(s, name, tiers) {
  if (name === '청교룡' && tiers.실체) {
    const 보너스 = 1 + 0.2 * s.법상_교혼;
    TRACE(s, 'OPT', `🐉청교룡·실체 종료 cleanup: 3명 280%×(1+0.2×교혼${s.법상_교혼}) = 280×${보너스.toFixed(2)} = ${(280*보너스).toFixed(0)}%`);
    const prev = s._currentSource; s._currentSource = `법상·${name}·종료`;
    record(s, 법상Dmg(s, 280 * 보너스), `법상·${name}(종료)`);
    s._currentSource = prev;
  }
  ...
  if (name === '청반룡' && tiers.실체) {
    TRACE(s, 'OPT', `🐉청반룡·실체 종료 cleanup: 용의 숨결 700% + 적 방어 -20% (20초)`);
    applyBuff(s, '법상청반_방어감소', { defDebuff: 20 }, 20);
    const prev = s._currentSource; s._currentSource = `법상·${name}·종료`;
  ...
  if (name === '청룡' && tiers.실체) {
    ...
    const prev = s._currentSource; s._currentSource = `법상·${name}·용의숨결`;
    record(s, 법상Dmg(s, 1500), `법상·${name}(종료)`);
    s._currentSource = prev;
  }
  if (name === '주작' && tiers.진령) {
    // 스펙: "적혼 1중첩당 무작위로 1-2개의 불깃털을 발사하여, 진원 10%의 확정 피해 (최대 8개 발사)"
    // 기댓값 모드: 1.5개 평균 / 랜덤 모드: 실제 1 또는 2 random — max 8 발 cap
    let 불깃털수 = 0;
    if (CFG.randomCrit) {
      for (let i = 0; i < s.법상_적혼; i++) 불깃털수 += (Math.random() < 0.5 ? 1 : 2);
      불깃털수 = Math.min(불깃털수, 8);
    } else {
      불깃털수 = Math.min(s.법상_적혼 * 1.5, 8);
    }
    if (불깃털수 > 0) {
  ...
  if (name === '봉황' && tiers.의념) {
    TRACE(s, 'OPT', `🦅봉황·의념 종료 cleanup: 봉황깃털 2개 (1개당 150% 확정) + 봉황 각인 +2`);
    const prev = s._currentSource; s._currentSource = `법상·${name}·봉황깃털종료`;
    for (let i = 0; i < 2; i++) {
      record(s, 법상Dmg(s, 150, { bypassDef: true }), `법상·${name}(의념)`);
      s.법상_봉황각인 = Math.min(s.법상_봉황각인 + 1, 5);
      applyBuff(s, '법상봉황_봉황각인', { dmgMult: 5, cat: 'dealt' }, 20, 5);
...
function 호신강기추가피해(s, pctOfJinwon, opts = {}) {
  if (!s.shieldRem || s.shieldRem <= 0) return null;  // 보호막 없으면 X
  return 진원피해(s, pctOfJinwon, 0, { ...opts, shieldOnly: true });
}
const TREASURES = {
  환음요탑: {
    name: '환음요탑',
...
const DEFENSE_TREASURES = {
  ...
  // 경화령도 는 기존 등록 (천강 세트 효과 = "가하는 다음 법보 +10%") 그대로 유지.
  // 인게임 현명 세트 경화령도+145 (= "받는 다음 법보 -10%") 는 별도 등록 X
  // — 세트 변경 메커니즘 미모델 (대부분 빌드는 한 세트만 사용)
};
const ALL_DEFENSE_TREASURES = Object.keys(DEFENSE_TREASURES);

// ======================== 세트 보너스 (3셋만) ========================
// 공격법보 3개 = 1세트 / 방어법보 3개 = 1세트 (각각 독립).
// 양쪽 모두 선택 시 = 두 효과 누적 (6셋 추가 보너스 X — 그냥 3셋 효과 두 번).
// 천강 (물리 계열): 물리 피해 심화 +5% (3셋)
// 현명 (술법 계열): 술법 피해 심화 +5% (3셋)
const TREASURE_SETS = {
  '단독': { name: '단독', desc: '세트 효과 없음', dmgIncPct: 0 },
  '천강': { name: '천강', desc: '천강 3셋 — 물리 피해 심화 +5%, 물리 피해 감면 +5%', dmgIncPct: 5 },
  '현명': { name: '현명', desc: '현명 3셋 — 술법 피해 심화 +5%, 술법 피해 감면 +5%', dmgIncPct: 5 },
};

// 선택된 방어법보 호신강기 합산 — 0개 선택 시 CFG.baseShield 폴백
function computeBaseShield(defenseTreasureList) {
  if (!Array.isArray(defenseTreasureList) || defenseTreasureList.length === 0) {
    return CFG.baseShield || 0;
  }
  let total = 0;
  for (const name of defenseTreasureList) {
    const dt = DEFENSE_TREASURES[name];
    if (dt) total += dt.shield;
  }
  return total;
}
...
// 방어법보 부활 체크 — 파괴된 법보의 CD (재사용 시간) 만료 시 보호막 풀로 재생성
// 매 event tick 마다 호출 (state.t 기준)
function 방어법보_부활체크(state) {
  if (!state.shields || state.shields.length === 0) return;
  for (const sh of state.shields) {
    if (!sh.destroyed) continue;
    const dt = DEFENSE_TREASURES[sh.name];
    const cd = (dt && dt.cd) || 180;
    const elapsed = state.t - (sh.destroyedT || 0);
    if (elapsed >= cd) {
      // 부활: 보호막 max 로 복원
      sh.destroyed = false;
      sh.rem = sh.max;
      sh.destroyedT = 0;
      TRACE(state, 'OPT', `🛡️ 방어법보 [${sh.name}] 부활 — 보호막 ${(sh.max/1e8).toFixed(2)}억 재생성 (CD ${cd}초 경과)`);
      // 합계 재계산
      state.shieldRem = state.shields.reduce((s, x) => s + (x.destroyed ? 0 : x.rem), 0);
    }
  }
...
function 방어법보_파괴효과(state, name) {
  ...
  // === 혼원마주 트리거 (방어법보 파괴 시 발동) ===
  // CD 160초, 첫 트리거 후 재진입 (즉, fired 후 next destroy까지 대기)
  // 적측 비술 (혼원) — 방어법보가 다시 살아나서 호신강기 회복
  // 너프 사양 (2026-05-09): 무 90% / 허 60% / 진 76% (이전 110% / 60% / 93%)
  const enemyBisul = (CFG.bisul && CFG.bisul.enemy) || [];
  for (const b of enemyBisul) {
    if (!b || b.master !== '혼원' || !b.branch) continue;
    const lastFire = state._혼원_lastT;
    if (lastFire !== undefined && state.t - lastFire < 160) continue; // CD
    // 평균 보호막값 계산
    if (!state.shields || state.shields.length === 0) continue;
    const avgShield = state.shields.reduce((s, sh) => s + sh.max, 0) / state.shields.length;
    const pct = b.branch === '무' ? 90 : (b.branch === '허' ? 60 : 76);
    const recoverShield = avgShield * (pct / 100);
    // 파괴된 법보 (현재 name 으로 들어온 것) 복구
    const target = state.shields.find(sh => sh.name === name);
    if (target) {
      target.destroyed = false;
      target.rem = recoverShield;
      target.destroyedT = 0;
      state.shieldRem = state.shields.reduce((s, sh) => s + (sh.destroyed ? 0 : sh.rem), 0);
      TRACE(state, 'OPT', `🔮혼원마주·${b.branch} 발동 (적): ${name} 복구 + 보호막 ${(recoverShield/1e8).toFixed(2)}억 (평균 ${(avgShield/1e8).toFixed(2)}억 × ${pct}%)`);
    }
    // 추가 효과 (branch 별)
    if (b.branch === '허') {
      // 12초간 신통 및 치명타 차단 +48% — sim 자기 신통 차단 + cr 감소 모델
      state.적_신통차단EndT = state.t + 12;
      applyBuff(state, '혼원허_치명타차단', { crRes: 48 }, 12);
      TRACE(state, 'OPT', `   └ 혼원·허 추가: 자기 신통 차단 12초 + cr 차단 48% 12초`);
    } else if (b.branch === '진') {
      // 사양 (신통_정리.md:812): "자신의 모든 방어법보의 미파괴 효과는 모든 방어법보가 파괴될 때까지 유지됩니다."
      //   = 마지막 1개 방어법보 살아있을 때까지 destroyed 된 법보들의 효과/buff 가 active 유지
      // sim 적용: 방어법보 효과/buff 는 newState 시 applyBuff 영구 active 라 destroyed 와 무관 — sim 결과 변경 없음
      //   shield reduction (명공현주 -15%, 성해천경 -10%, 경화령도) 도 destroyed 시 hit 자체 안 들어가니 적용 X (rem 0 → 흡수 0)
      // → applyBuff 등록만 (사양 표시용, sim dispatch 미사용 — 결과 영향 X)
      applyBuff(state, '혼원진_미파괴효과유지', { 미파괴효과: true }, 1e9);
      TRACE(state, 'OPT', `   └ 혼원·진 추가: 모든 방어법보 파괴까지 미파괴 효과 유지 (applyBuff 등록 — sim 결과 영향 X: 방어법보 buff 는 newState 영구 active, shield rem 0 = 흡수 0)`);
    }
    state._혼원_lastT = state.t;
    break; // 한 발동만
  }
}
...
function getShieldReduction(state, shieldName, isShintong) {
  ...
  if (shieldName === '성해천경') return 0.10;
  if (shieldName === '경화령도') return get경화령도Stack(state) * 0.05;
  return 0;
}
...
function applyShieldDamage(state, rawDmg, opts = {}) {
  ...
  for (const sh of state.shields) {
    ...
    // 흡수된 effective 에 해당하는 raw 만큼 remaining 차감
    const consumedRaw = reduction > 0 ? absorbed / (1 - reduction) : absorbed;
    remaining -= consumedRaw;
...
// 인간계 유파 — 본 신통 데미지에 CFG.인간계보너스 추가 적용 (복룡/중광/이화/천로/오뢰/신소)
const 인간계_유파 = new Set(['복룡', '중광', '이화', '천로', '오뢰', '신소']);
...
function emitSnapTrace(state) {
  ...
  if (state.nextCast) {
    ...
    if (ncSnap.consumedSources && ncSnap.consumedSources.length > 0) {
      ...
      // ncSnap (소비된 nextCast) push 안 함 — consumedSources 가 비어있을 때만 fallback 동작이라 일반적으로 도달 X
    }
    // localFinalDmg (본 cast 한정 — 붕산 통백 +20 등) 는 push — 그 cast 자체에 적용된 효과라 셀로 표시
  ...
  const snap = {
    ...
    bd,
    src: state._currentSource,  // SNAP 매핑용 cast 식별자 (UI 측 castRaws 매칭)
  };
  if (CFG.debugDealDamage) console.log('[DBG][SNAP] t=' + state.t.toFixed(2) + ' src=' + state._currentSource + ' finalDmg=' + snap.finalDmg.toFixed(1) + ' nc=' + ((state.nextCast && state.nextCast.finalDmg) || 0).toFixed(1) + ' ncSnap=' + ((state._snapNextCastConsumed && state._snapNextCastConsumed.finalDmg) || 0).toFixed(1));
...
function simulateBuild(build, treasures, orderOverride, skillsOverride, opts) {
  ...
  state.defenseTreasures = (opts && Array.isArray(opts.defenseTreasures)) ? opts.defenseTreasures.slice() : [];
  ...
  if (state.shields.length === 0 && (CFG.baseShield || 0) > 0) {
    state.shields.push({ name: '_legacy', max: CFG.baseShield, rem: CFG.baseShield, destroyed: false, destroyedT: 0 });
  }
  // 천창홍고 passive: 다른 방어법보 max +15% (전투 시작 시 1회)
  if (state.shields.some(sh => sh.name === '천창홍고')) {
    for (const sh of state.shields) {
      if (sh.name !== '천창홍고') {
        sh.max = sh.max * 1.15;
        sh.rem = sh.max;
  ...
  state.attackSetMode = (opts && opts.attackSetMode) || '단독';
  state.defenseSetMode = (opts && opts.defenseSetMode) || '단독';
  const _setToAttr = { 천강: '물리', 현명: '술법' };
  if (state.attackSetMode !== '단독' && TREASURE_SETS[state.attackSetMode]) {
    const pct = TREASURE_SETS[state.attackSetMode].dmgIncPct;
    const attr = _setToAttr[state.attackSetMode];
    if (pct > 0 && attr) applyBuff(state, `세트·공격·${state.attackSetMode}`, { dmgMult: pct, cat: 'inc', attr }, 1e9);
  }
  if (state.defenseSetMode !== '단독' && TREASURE_SETS[state.defenseSetMode]) {
    const pct = TREASURE_SETS[state.defenseSetMode].dmgIncPct;
    const attr = _setToAttr[state.defenseSetMode];
    if (pct > 0 && attr) applyBuff(state, `세트·방어·${state.defenseSetMode}`, { dmgMult: pct, cat: 'inc', attr }, 1e9);
  }
  ...
  state._식혼_fired = false;
  // 자기 탁천·허 ② 차회 광역 발동권 buff (사양: 최대 2회 발동)
  // 마스터 발동 시: ② 즉시 1회 광역 emit + 차회광역 buff 생성 (= true)
  // 매 다음 HP 0 도달 시 (마스터 미발동 — CD 안): buff 가 광역 1회 emit + buff 소진 (= false)
  // → 한 마스터 발동 cycle 당 총 2회 (즉시 1회 + 다음 1회)
  state.탁천허_차회광역 = false;
  ...
  for (const ev of events) {
    ...
    // 방어법보 부활 처리 — CD (재사용 시간 180초) 만료 시 보호막 재생성
    방어법보_부활체크(state);
    ...
    if (state.관일End > 0 && state.t >= state.관일End && !state.관일종료처리) {
      ...
      for (let i = 0; i < js; i++) {
        record(state, dealDamage(state, 30, { noSkillMult: true, type: '호무', attr: '물리' }), `천연(호무) ${i+1}/${js}`);
      }
    ...
    if (state.독주FireT > 0 && state.t >= state.독주FireT) {
      ...
      if (doksu > 0) {
        ...
        record(state, dealDamage(state, doksu, { noSkillMult: true, attr: '물리' }), '독주');
        // [고담] 독주 발동 시점에 계약 3중첩 이상이면 180% 추가 물리 (max tier)
        if (합 >= 3) {
          state._currentSource = '고담(독주+3중첩)';
          TRACE(state, 'OPT', `🟠고담 발동: 독주+계약 ${합}중첩 → 180% 물리`);
          record(state, dealDamage(state, 180, { noSkillMult: true, attr: '물리' }), '고담');
    ...
    if (state.수광End > 0 && state.t >= state.수광End && !state.수광종료처리) {
      ...
      if (천붕 > 0) {
        ...
        record(state, dealDamage(state, 천붕, { noSkillMult: true, attr: '물리' }));
        state._currentSource = prev;
      }
      state.수광종료처리 = true;
    }
    ...
    if (ev.kind === 'skill') {
      ...
      if (sk) {
        ...
        // 첫 신통 시각 기록 — 경화령도 stack 계산용
        if (state._첫신통시각 === undefined) state._첫신통시각 = state.t;
        // 분신용: 이 cast 시작 시점의 buff key 집합 — clone 계산 시 "이 cast 가 추가한 self-buff" 만 적용하기 위함
        state._castStartBuffKeys = new Set(state.buffs.map(b => b.key));
        ...
        if (sk.name === '형혹·흑성' || state.buffs.some(b => b.key === '형혹흑성_혹성' && b.endT > state.t)) {
          if (state.selectedSkills && state.selectedSkills.has('형혹·흑성')) {
            ...
            record(state, dealDamage(state, 20, { noSkillMult: true, attr: '물리' }));
          }
        }
        ...
        if (famActive(state, '주술') && (state.유식남은 || 0) > 0 && state.selectedSkills && state.selectedSkills.has('주술·유식')) {
          ...
          record(state, dealDamage(state, 30, { noSkillMult: true, attr: '물리' }), '유식');
          state._currentSource = prev;
        }
        ...
        if (state.famSlots.옥추 && state.buffs.some(b => b.key === '옥추수광_수광' && b.endT > state.t)) {
          if (!state._수광이미처리) {
            ...
            record(state, dealDamage(state, 30, { noSkillMult: true, attr: '물리' }));
          }
        }
        if (state.famSlots.참허 && state.분광End > 0 && state.t < state.분광End - 0.1) {
          if (!state._분광이미처리) {
            ...
            record(state, dealDamage(state, 24, { noSkillMult: true, type: '호무', attr: '물리' }));
          }
        }
        // === cast 실행 ===
        ...
        if (state.관일End > 0 && state.t < state.관일End - 0.1) {
          if (!state._관일이미처리) {
            ...
            record(state, dealDamage(state, 40, { noSkillMult: true, type: '호무', attr: '물리' })); // max tier: 40% 호무
        ...
        if (state.famSlots.중광 && state.검광End > 0 && state.t < state.검광End - 0.1) {
          if (!state._검광이미처리) {
            ...
            record(state, dealDamage(state, 23, { noSkillMult: true, type: '호무', attr: '물리' }));
            state._currentSource = prev;
          ...
          if (CFG.randomCrit) {
            ...
            if (trigCount > 0) {
              ...
              for (let i = 0; i < trigCount; i++) {
                ...
                record(state, dealDamage(state, 8, { noSkillMult: true, attr: '물리' }));
              }
            }
          } else {
            const crEff = Math.min(100, CFG.baseCR * (1 + sumBuffCR(state) / 100) * (1 + state.nextCast.finalCR / 100) * (1 + sumBuffCritRes(state) / 100)) / 100;
      ...
      if (현재진원 < 소모량 - 0.001) {
        ...
        state._currentTreasure = trName;  // attr 추론용 (법보별 물리/술법 구분)
        // 다음 법보 보너스 (경화령도/명공현주 파괴 시 +10% finalDmg) — 한 번만 소비
        const _법보보너스 = state.다음법보_보너스 || 0;
        if (_법보보너스 > 0) {
          state.nextCast = state.nextCast || {};
          state.nextCast.finalDmg = (state.nextCast.finalDmg || 0) + _법보보너스;
          TRACE(state, 'OPT', `🛡️ 방어법보 파괴 보너스 발동: 다음 법보 +${_법보보너스}% (소비)`);
          state.다음법보_보너스 = 0;
        }
        // 오염혁선 효과: 다음 법보 확정 치명타 — 법보 type='법보절대' 에 적용 (state.법보_확정크리)
        const _오염혁선크리 = state.오염혁선_확정크리;
        if (_오염혁선크리) {
          state.법보_확정크리 = true;
          TRACE(state, 'OPT', `🔮 오염혁선 확정 치명타 발동: 다음 법보 cr 100% (소비)`);
          state.오염혁선_확정크리 = false;
        }
        ...
        if (_법보보너스 > 0 && state.nextCast) {
          state.nextCast.finalDmg = Math.max(0, (state.nextCast.finalDmg || 0) - _법보보너스);
        }
